...
class CausalQNet(DistilBertPreTrainedModel):
    ...
    def forward(self, text_ids, text_len, text_mask, A, Y, use_mlm=True):
        ...
        if use_mlm:
            ...
            prediction_logits = nn.functional.gelu(prediction_logits)  # (bs, seq_length, dim)
...
class QNet:
    ...
    def get_pA(self, texts, confounders=None):
        """
        Returns an array of predicted probabilities for A=1,
        using the learned DistilBert + linear head (self.g_hat).
        """

        # We'll create a dummy `treatments` and `outcomes` if needed
        # because we only need text (and maybe confounders) to get A.
        if confounders is None:
            confounders = [0]*len(texts)
        treatments = [0]*len(texts)  # not used for p(A=1)
        outcomes = [0]*len(texts)    # not used for p(A=1)
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="Predicting p(A=1)"):
                if CUDA:
                    batch = (x.cuda() for x in batch)
                text_id, text_len, text_mask, A, C, Y = batch

                # Forward pass (but we only need pooled_output -> g_hat -> softmax)
                bert_outputs = self.model.distilbert(
                    input_ids=text_id, 
                    attention_mask=text_mask
                )
                seq_output = bert_outputs[0]       # (batch_size, seq_len, hidden_dim)
                pooled_output = seq_output[:, 0]   # CLS token

                # A logits => shape [batch_size, 2]
                a_logits = self.model.g_hat(pooled_output)  
                # Softmax => shape [batch_size, 2]
                a_probs = torch.nn.functional.softmax(a_logits, dim=1)  
                
                # Probability of A=1 => a_probs[:,1]
                pA1 = a_probs[:, 1].detach().cpu().numpy()
                all_probs.extend(pA1)
